function dd = DepthBasedTreeGWVV_L1_AlignedTree(TM, pathXX, pathZZ, weightXX, weightZZ, TTXX, TTZZ, idXX, idZZ, ww)
...
while prob_ID <= length_queueProb
    ...
    if (simpleFlagX + simpleFlagZ) == 0
        [ii_dd, ii_TT] = L1_OT1D_Depth(DX, DZ, WX, WZ);
        dd = dd + ii_ww*ii_dd;
        ii_TT = ww*ii_TT;
    ...
    if (simpleFlagX + simpleFlagZ) == 1
        if simpleFlagX == 1 % XX
            dd = dd + ww*L1_OT1D_Depth_Simple(TM, pathZZ, weightZZ, ii_idZZ);
        else % ZZ
            dd = dd + ww*L1_OT1D_Depth_Simple(TM, pathXX, weightXX, ii_idXX);
        end
    end
